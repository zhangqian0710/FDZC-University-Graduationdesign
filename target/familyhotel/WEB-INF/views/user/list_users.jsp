...
<head>
    ...
    <script>
        //removeModal
        function showRemoveDialog(userno,userid){
            $('#removeUserId').text(userid);
            $('#removeMsgUser').text('确定要删除【'+userno+'】编号的管理员吗？');

        }

        //removeUser
        function removeUser(userId){
            location.href='<c:url value="/user/removeUser/"/>'+ userId;
        }

        //updataUser
        function updateUser(userId){
            location.href='<c:url value="/user/updateUser/"/>' + userId;
        }
                ...
                <div class="card card-shadow mb-4">
                    <div class="card-header">
                        <div class="card-title" style="font-size:25px;">
                            <div style="float: left">管理员列表</div>
                            ...
                            <div  style="float: right;height: 15px; margin-right: 10px; margin-top:5px;" >
                                <form action="${path}/user/listUser" method="post" cssClass="col-12" modelAttribute="user" >
                                    <div class="form-row">
                                        <p style="font-size: 18px; margin-top: 1%;">管理员类型：</p>
                                        <select id="a" name="userLimit" class="form-control" style="width: 120px;">
                                            <option value="" selected>全部</option>
                                            <option value="a" <c:if test="${user.userLimit=='a'}">selected</c:if>>超级管理员</option>
                                            <option value="b" <c:if test="${user.userLimit=='b'}">selected</c:if>>前台服务员</option>
                                            <option value="c" <c:if test="${user.userLimit=='c'}">selected</c:if>>财务管理员</option>
                                        </select>&nbsp;&nbsp;
                    ...
                    <div class="card-body">
                        <table id="bs4-table" class="table table-bordered table-striped" cellspacing="0" width="100%">
                            <thead>
                                <th style="width:15%; text-align:center">管理员编号</th>
                                <th style="width:20%; text-align:center">管理员账号</th>
                                <th style="width:20%;text-align:center">管理员姓名</th>
                                <th style="width:10%; text-align:center">性别</th>
                                <th style="width:20%; text-align:center">管理员类别</th>
                            ...
                            <tr>
                                ...
                                <td style="text-align:center;vertical-align:middle;">${user.userNo}</td>
                                <td style="text-align:center;vertical-align:middle;">${user.userName}</td>
                                <td style="text-align:center;vertical-align:middle;">
                                        <%--111 ${user.userName} ${user.userSex}--%>
                                        <c:choose>
                                            <c:when test="${user.userSex eq 'a'}">男  <i class="fa fa-male fa-lg"></i></c:when>
                                            <c:when test="${user.userSex eq 'b'}">女  <i class="fa fa-female fa-lg"></i></c:when>
                                        </c:choose>
                                </td>
                                <td style="text-align:center;vertical-align:middle;">
                                    <c:choose>
                                        <c:when test="${user.userLimit eq 'a'}">超级管理员 </c:when>
                                        <c:when test="${user.userLimit eq 'b'}">前台服务员 </c:when>
                                        <c:when test="${user.userLimit eq 'c'}">财务管理员 </c:when>
                                    </c:choose>
                                </td>
                                <td style="text-align:center;vertical-align:middle;">
                                    <button class="btn btn-sm btn-warning" onclick="updateUser(${user.userId})"> 修 改 </button>
                                    <button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#removeRoomModal" onclick="showRemoveDialog('${user.userNo}',${user.userId})"> 删 除 </button>
                                </td>
                            </tr>
                            </c:forEach>
                            </tbody>
                            ...
                            <div class="col-md-6" style="float: left;width: 300px; ">
                                <nav aria-label="Page navigation">
                                    <ul class="pagination" style="float: right; ">
                                        <li><a href="${path}/user/listUser?pn=1"> <button class="btn btn-sm btn-outline-success">首页</button>&nbsp;</a></li>
                                        <c:if test="${pageInfo.hasPreviousPage }">
                                            <li>
                                                <a href="${path}/user/listUser?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                                        ...
                                        <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
                                            ...
                                            <c:if test="${page_Num != pageInfo.pageNum }">
                                                <li><a href="${path}/user/listUser?pn=${ page_Num}"><button class="btn btn-sm btn-outline-success">${ page_Num}</button>&nbsp;</a></li>
                                            </c:if>
                                        </c:forEach>
                                        <c:if test="${pageInfo.hasNextPage }">
                                            <li>
                                                <a href="${path}/user/listUser?pn=${pageInfo.pageNum+1}" aria-label="Next">
                                                    <button class="btn btn-sm btn-outline-success">&raquo;</button>&nbsp;
                                                </a>
                                            </li>
                                        </c:if>
                                        <li><a href="${path}/user/listUser?pn=${pageInfo.pages}"> <button class="btn btn-sm btn-outline-success">末页</button>&nbsp;</a></li>
                                    </ul>
                                </nav>
                            </div>
                        </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p id="removeMsgUser"></p>
                    <span id="removeUserId" style="display:none"></span>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">放弃操作 </button>
                    <button type="button" class="btn btn-primary" onclick="removeUser($('#removeUserId').text());">确认删除</button>
                </div>
            </div>
        </div>
    </div>
